...
class Simulation:
    ...
    def __init__(self, agent_num, bfi_type, random_seed=None, model=None, result_dir="result/demo"):
        ...
        self.profile = self.setup_profile(specific_bfi_type=bfi_type)
        print('Setup profile')
    ...
    def setup_profile(self, specific_bfi_type=None):
        """Set up agent profiles including CEO and shareholders.

        Args:
            specific_bfi_type: Optional OCEAN type to use for all agents (e.g., "oCeAn")
        """
        profile = dict()

        # If a specific OCEAN type is specified, use it for all agents
        if specific_bfi_type:
            bfi_type_for_experiment = specific_bfi_type
        else:
            # Default to a type if none is specified, as this is a 'same personality' script
            bfi_type_for_experiment = 'oCeAn'

        profile['all'] = ChooseProfile(
            agent_num=self.agent_num,
            bfi_type=bfi_type_for_experiment,
            random_seed=self.random_seed
        )

        # The 'ocean_trait' from the data will be used in prompts, not the 'type'.
        # The ocean_trait key already exists in the data, so no need to modify it

        profile['shareholders'] = profile['all'].copy()

        # Select CEO from shareholders
...
def run_single_bfi_simulation(bfi_type, random_seed=42, num_agents=6, years=1):

    result_dir = f"result/Homogeneous"

    print(f"Starting simulation for OCEAN: {bfi_type}")
    ...
    # Print summary for this OCEAN
    print(
        f"OCEAN {bfi_type} completed - Final assets: ${results['final_assets']:.2f}, Growth: {results['growth_percentage']:.2f}%")

    time.sleep(60)

    return bfi_type, results


def main():
    # Define all OCEAN types to simulate
    all_bfi_types = [
    ...
    print(f"Running {len(all_bfi_types)} simulations in parallel...")

    # Run all OCEAN simulations in parallel
    all_results = {}

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_bfi = {
            executor.submit(run_single_bfi_simulation, bfi, random_seed, num_agents, years): bfi
            for bfi in all_bfi_types
        }

        # Collect results as they complete
        for future in concurrent.futures.as_completed(future_to_bfi):
            bfi = future_to_bfi[future]
            try:
                bfi_type, results = future.result()
                all_results[bfi_type] = results
                print(f"Successfully completed simulation for {bfi_type}")
            except Exception as exc:
                print(f"OCEAN {bfi} generated an exception: {exc}")

    # Print summary of all results
    print("\n=== FINAL SUMMARY ===")
    print("OCEAN Type | Final Assets | Growth Rate")
    print("-" * 45)
    for bfi, results in sorted(all_results.items()):
        print(f"{bfi:8} | ${results['final_assets']:11.2f} | {results['growth_percentage']:6.2f}%")
