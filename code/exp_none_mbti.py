...
class Simulation:
    ...
    def setup_profile(self, specific_mbti=None):
        """Set up agent profiles including CEO and shareholders.

        For baseline experiment: randomly selects profiles but doesn't use MBTI in decision-making
        """
        profile = dict()

        # For baseline experiment: simply randomly select profiles without MBTI consideration
        # We still need to load profiles for names and other basic info
        root_dir = os.getcwd()
        with open(root_dir + '/data/mbti_1024_bank_new.json', 'r+', encoding='utf-8') as f:
        ...
        # Randomly select agent profiles without any MBTI filtering
        profile['all'] = random.sample(data_list, self.agent_num)
...
def run_baseline_simulation(random_seed=42, num_agents=6, years=1):
    """Run a single baseline simulation without specific MBTI personalities"""
    # Create result directory for baseline
    result_dir = "result/baseline"

    print("Starting baseline simulation (no MBTI specifications)")

    # Create and run simulation without specific MBTI
    simulation = Simulation(
        agent_num=num_agents,
        random_seed=random_seed,
        model=model,
        result_dir=result_dir,
        specific_mbti=None  # No specific MBTI ensures random profiles are selected
    )
    ...
    # Print summary
    print(
        f"Baseline simulation completed - Final assets: ${results['final_assets']:.2f}, Growth: {results['growth_percentage']:.2f}%")
...
def main():
    ...
    print("Running baseline simulation without MBTI specifications...")

    # Run baseline simulation
    start_time = time.time()
    results = run_baseline_simulation(random_seed, num_agents, years)
    end_time = time.time()
    ...
    print(f"Final Assets: ${results['final_assets']:.2f}")
    print(f"Growth Rate: {results['growth_percentage']:.2f}%")
    print(f"Simulation took {hours}h {minutes}m {seconds:.2f}s.")
