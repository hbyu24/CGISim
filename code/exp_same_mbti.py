...
class Simulation:
    ...
    def setup_profile(self, specific_mbti=None):
        ...
        # If a specific MBTI is specified, use it for all agents
        if specific_mbti:
...
def run_single_mbti_simulation(mbti_type, random_seed=42, num_agents=6, years=1):
    """Run a single simulation for a specific MBTI type"""
    # Create unique result directory for this MBTI
    result_dir = f"result/exp_same_mbti"

    print(f"Starting simulation for MBTI: {mbti_type}")
    ...
    # Print summary for this MBTI
    print(
        f"MBTI {mbti_type} completed - Final assets: ${results['final_assets']:.2f}, Growth: {results['growth_percentage']:.2f}%")
...
def main():
    ...
    print(f"Running {len(all_mbti_types)} simulations in parallel...")

    # Run all MBTI simulations in parallel
    all_results = {}

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_mbti = {
            executor.submit(run_single_mbti_simulation, mbti, random_seed, num_agents, years): mbti
            for mbti in all_mbti_types
        }

        # Collect results as they complete
        for future in concurrent.futures.as_completed(future_to_mbti):
            mbti = future_to_mbti[future]
            try:
                mbti_type, results = future.result()
                all_results[mbti_type] = results
                print(f"Successfully completed simulation for {mbti_type}")
            except Exception as exc:
                print(f"MBTI {mbti} generated an exception: {exc}")

    # Print summary of all results
    print("\n=== FINAL SUMMARY ===")
    print("MBTI Type | Final Assets | Growth Rate")
    print("-" * 45)
    for mbti, results in sorted(all_results.items()):
        print(f"{mbti:8} | ${results['final_assets']:11.2f} | {results['growth_percentage']:6.2f}%")
