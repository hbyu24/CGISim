...
class SimulationResultSaver:
    ...
    def __init__(self, base_dir="result/demo"):
        """
    ...
    def create_simulation_folder(self, num_agents,ceo_bfi=None):
        """
        ...
        safe_ceo_bfi = re.sub(r'[^a-zA-Z0-9_.-]', '_', str(ceo_bfi)) if ceo_bfi else "no_bfi"
        folder_name = f"{timestamp}_agents{num_agents}_{safe_ceo_bfi}"
        ...
        try:
            ...
            <body>
                ...
                <div class="ceo-info">
                    ...
                    <p><strong>BFI:</strong> {data['CEO']['BFI']}</p>
                </div>
            ...
            for shareholder_name, shareholder_data in data.get('shareholders', {}).items():
                html_content += f"""
                <div class="shareholder-info">
                    <p><strong>Name:</strong> {shareholder_name}</p>
                    <p><strong>BFI:</strong> {shareholder_data.get('BFI', 'N/A')}</p>
                </div>
                """
