...
Model_And_Embedder = BasicUtils(['qwen3-235b-a22b', 'gpt-4.1-mini', 'gpt-4.1-nano'],disable_language_model)
model = Model_And_Embedder.model_list['gpt-4.1-mini']
model_high=Model_And_Embedder.model_list['qwen3-235b-a22b']
...
"""
    ...
    def finalize_meeting(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(agent_items)/2) as executor:
...
After you've submitted your assessments, I'll address the key themes and outline my thoughts on how we can improve in the coming year."""
    ...
    =====================""",
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(shareholder_items)/2) as executor:
...
=====================""",
    ...
    =====================""",
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(shareholders_data)/2) as executor:
...
def run_team_composition_simulation(team_name, team_config, random_seed, num_agents, years, result_dir):
    ...
    # Get model_high's name for folder naming
    if disable_language_model == True:
        model_high_name='None'
    else:
        model_high_name = model_high._model_name if 'model_high' in globals() else "unknown"

    # Create the simulation folder with model_high name included
    ceo_mbti = simulation.ceo.config.traits
    simulation.simulation_folder = simulation.result_saver.create_simulation_folder(num_agents,
                                                                                    f"{ceo_mbti}_{model_high_name}")
...
def main():
    ...
    result_dir = "result/exp_llm/qwen"  # New directory for team composition results
