...
class BasicUtils:
    """
    A simplified utility class for initializing the gpt-4.1 series models
    and a text embedder required for the simulation. This version removes all
    sensitive information and unused models to comply with anonymous review
    requirements.
    """

    def __init__(self, disable_language_model=False):
        """
        Initializes the embedder and the three core GPT models.
        API keys are loaded from environment variables to ensure security and anonymity.
        """
        # 1. Initialize the text embedder
        self.embedder = self._embedder_setup()

        # 2. Define model configurations
        # Map model names to their corresponding API key environment variables
        self.model_configs = {
            'gpt-4.1': 'GPT41_API_KEY',
            'gpt-4.1-mini': 'GPT41_MINI_API_KEY',
            'gpt-4.1-nano': 'GPT41_NANO_API_KEY',
        }

        # 3. Directly initialize the three models used in the code
        self.model_high = self._model_setup('gpt-4.1', disable_language_model)
        self.model = self._model_setup('gpt-4.1-mini', disable_language_model)  # 'model' defaults to gpt-4.1-mini
        self.model_low = self._model_setup('gpt-4.1-nano', disable_language_model)

        # 4. Create a dictionary of models for retrieval by name (if needed)
        self.model_list = {
            'gpt-4.1': self.model_high,
            'gpt-4.1-mini': self.model,
            'gpt-4.1-nano': self.model_low,
        }

    def _embedder_setup(self):
        """Sets up the sentence-transformer embedding model."""
        st_model = sentence_transformers.SentenceTransformer(
            'sentence-transformers/all-mpnet-base-v2')
        embedder = lambda x: st_model.encode(x, show_progress_bar=False)
        return embedder

    def _model_setup(self, model_name, disable_language_model=False):
        """
        Initializes an OpenAI model based on the model name, using an API key
        ...
        if model_name in self.model_configs:
            env_var_name = self.model_configs[model_name]
            api_key = os.environ.get(env_var_name)

            if not api_key:
                print(
                    f"Warning: Environment variable {env_var_name} is not set. Model '{model_name}' will not be available.")

            return utils.language_model_setup(
                api_type='openai',
                model_name=model_name,
                api_key=api_key,  # Securely load from environment variable
    ...
    def get_model(self, model_name):
        """Gets an initialized model by its name."""
...
# Disable language model (for debugging, set to False for normal runs)
disable_language_model = False

# Initialize the utility class
Model_And_Embedder = BasicUtils(disable_language_model)

# Directly get models from the utility instance
model = Model_And_Embedder.model  # gpt-4.1-mini
model_high = Model_And_Embedder.model_high  # gpt-4.1
model_low = Model_And_Embedder.model_low  # gpt-4.1-nano
...
print("Model and embedder initialized successfully.")
...
Your decisions should authentically reflect your OCEAN personality type - whether you're analytical, emotional, cautious, or bold in your approach. Consider how your personality would naturally respond to investment opportunities, risk levels, and interpersonal dynamics in these meetings. """
...
def ChooseProfile(agent_num, bfi_type, random_seed=None):
    """Chooses agent profiles based on OCEAN personality type."""
    # Construct the absolute path to the data file
    root_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(root_dir, 'OCEAN_Data.json')

    with open(data_path, 'r', encoding='utf-8') as f:
        data_list = json.load(f)
    
    # Filter profiles by the specified OCEAN type
    filtered_profiles = [item for item in data_list if item['type'] == bfi_type]

    if not filtered_profiles:
        print(f"No profiles found for OCEAN type: {bfi_type}")
        # Show available types for debugging
        available_types = set(item['type'] for item in data_list)
        print(f"Available types: {available_types}")
        return []

    if len(filtered_profiles) < agent_num:
        print(f"Warning: Not enough profiles for OCEAN type {bfi_type}. Required: {agent_num}, Found: {len(filtered_profiles)}.")
        # Optionally, you could decide to return what you have, or raise an error.
        # For now, let's return all available profiles of that type.
    ...
    selected_profiles = random.sample(filtered_profiles, agent_num)
    return selected_profiles


# %%
def SetupLeader(selected_profiles, random_seed=None):
    """Randomly selects a leader from the list of profiles."""
    if random_seed is not None:
        random.seed(random_seed)
    
    if not selected_profiles:
        print("Warning: The list of selected profiles is empty. Cannot select a leader.")
        return None
...
class Shareholder:
    ...
    def setup_agent_config(self, profile):
        agent_config = formative_memories.AgentConfig(
            ...
            traits=profile['ocean_trait'],
            extras={
                'leader': False,
                'type': profile['type'], # Keep the short type for potential filtering
    ...
    def setup_memory(self):
        ...
        profile_list = ['personality', 'experience']
        for index in profile_list:
            if index == 'personality':
                content = self.config.extras['personality']
                index = index + '_BFI'
                agent_memory.add(text=content, tags=index, importance=1)
            else:
                content = self.config.extras[index]
                agent_memory.add(text=content, tags=index, importance=0.9)
    ...
    def setup_components(self, measurements: measurements_lib.Measurements | None = None):
        constant_personality = agent_components.constant.Constant(
            state=self.config.extras['personality'],
            pre_act_key='Personality of OCEAN'
        )
        constant_experience = agent_components.constant.Constant(
            state=self.config.extras['experience'],
            pre_act_key='Personal Experience'
        )
        ...
        SelfPerception = question_of_recent_memories.SelfPerception(
            ...
            pre_act_key='\nSelfPerception',
            clock_now=self.clock.now,
            components={'Personality of OCEAN': constant_personality},
        ...
        all_components = {
            ...
            'Personality of OCEAN': constant_personality,
            'Personal Experience': constant_experience,
...
class Company:
    def __init__(self, shareholders, ceo, market, prompts, initial_assets=100000, model=None,simulation=None):
        ...
        self.decision_tracker = DecisionTracker(ceo.config.name, ceo_bfi=ceo.config.traits, shareholders=shareholders, ceo=ceo)
...
"""
    ...
    def generate_meeting_reflection(self, agent, doc):
        ...
        reflection_prompt = f"""As {agent_name} ({role}) with Big Five personality traits {mbti_type}, create a structured reflection on this meeting.
...
class DecisionTracker:
    ...
    def __init__(self, ceo_name, ceo_bfi, shareholders,ceo):
        """Initialize the decision tracking structure."""
        self.data = {
            "CEO": {
                "name": ceo_name,
                "OCEAN": ceo_bfi,
                'type':ceo.config.extras['type'],
                "themes": {},
                "discussion_phase": {},
                "personal_summary": {},
                "personal_reflection": {},
        ...
        for shareholder_name, shareholder in shareholders.items():
            self.data["shareholders"][shareholder_name] = {
                ...
                "OCEAN": shareholder.config.traits,
                'type':shareholder.config.extras['type'],
                "discussion_phase": {},
                "personal_summary": {},
                "personal_reflection": {},
                "personal_position": {},
    ...
    def record_shareholder_review(self, meeting_id, shareholder_name, feedback, ratings):
        ...
        if shareholder_name in self.data["shareholders"]:
            ...
            # Ensure the meeting entry exists
            if meeting_id not in self.data["shareholders"][shareholder_name]["review_meeting"]:
                self.data["shareholders"][shareholder_name]["review_meeting"][meeting_id] = {
            ...
            if feedback is not None:
                self.data["shareholders"][shareholder_name]["review_meeting"][meeting_id]["written_feedback"] = feedback

            # Update ratings if provided
            if ratings is not None:
                self.data["shareholders"][shareholder_name]["review_meeting"][meeting_id]["ratings"] = ratings
    ...
    def validate_data_integrity(self):
        ...
        for shareholder_name, shareholder_data in self.data["shareholders"].items():
            if not shareholder_data.get("OCEAN"):
                validation_results["valid"] = False
                validation_results["issues"].append(f"OCEAN missing for {shareholder_name}")
    ...
    def get_agent_summary(self, agent_name):
        ...
        # Check if agent is shareholder
            ...
            agent_summary["basic_info"] = {
                "role": "Shareholder",
                "bfi": self.data["shareholders"][agent_name]["OCEAN"]
            }
...
- EMBODY YOUR PERSONALITY TYPE: Let your Big Five personality traits naturally influence your leadership style, information processing approach, and communication patterns throughout the discussion.
...
- EMBODY YOUR PERSONALITY TYPE: Let your Big Five personality traits naturally influence which aspects of the discussion draw your attention and how you process information.
...
"""
            ...
            if year > 1:
                ...
                if prev_year_performance:
                    ...
                    analysis_prompt = entity.free_action_spec(
                        call_to_action=f"As {self.company.ceo.config.name}, provide a 2-3 sentence analysis of the previous year's performance. Previous year data: {escaped_performance}. Consider your Big Five personality traits when crafting this response.",
    ...
    def _form_initial_position_for_ceo(self, meeting_id, meeting_type, options, market_condition,
        ...
        if "annual_budget" in meeting_type:
            ...
            preference_prompt = f"""You are {self.company.ceo.config.name}, the CEO of the investment company with Big Five personality traits {self.company.ceo.config.traits}.
...
Based on your Big Five personality traits ({self.company.ceo.config.traits}), the current market conditions, and available data, assess your confidence in each budget allocation option and select which you personally believe would be best for the company this year.
...
REASONING: [explain your reasoning in 3-5 sentences that reflect your Big Five personality traits]
...
Based on your Big Five personality traits ({self.company.ceo.config.traits}), the current market conditions, and available information, assess your confidence in each asset option and select which you personally believe would be the best investment for this quarter.
IMPORTANT: Frame your thinking in first person ("I/my") as an internal reflection process. Use step-by-step analysis (think through each element of your decision sequentially) to demonstrate your reasoning process. Express your analytical considerations and personal assessment with authentic introspection.
IMPORTANT: This initial position will guide your leadership throughout the discussion. While you remain open to changing your mind if compelling arguments arise, you should advocate for this position and not abandon it easily.
...
===MEETING CONTEXT==="""
    ...
    def _form_initial_position_for_shareholder(self, shareholder, meeting_id, meeting_type, options, market_condition,
        ...
        if "annual_budget" in meeting_type:
            ...
            preference_prompt = f"""You are {shareholder.config.name}, a shareholder in the investment company with Big Five personality traits {shareholder.config.traits}.
...
Based on your Big Five personality traits ({shareholder.config.traits}), the current market conditions, and available data, assess your confidence in each budget allocation option and select which you personally believe would be best for the company this year.
IMPORTANT: Frame your thinking in first person ("I/my") as an internal reflection process. Use step-by-step analysis (think through each element of your decision sequentially) to demonstrate your reasoning process. Express your analytical considerations and personal assessment with authentic introspection.
...
Based on your Big Five personality traits ({shareholder.config.traits}), the current market conditions, and available information, assess your confidence in each asset option and select which you personally believe would be the best investment choice for this quarter.
IMPORTANT: Frame your thinking in first person ("I/my") as an internal reflection process. Use step-by-step analysis (think through each element of your decision sequentially) to demonstrate your reasoning process. Express your analytical considerations and personal assessment with authentic introspection.
IMPORTANT: This initial position will guide your participation throughout the discussion. While you should remain open to changing your mind if truly compelling arguments are presented, you should generally advocate for this position based on your personality traits.
...
"""
            ...
            outline_prompt = f"""You are {self.company.ceo.config.name}, the CEO of the investment company with Big Five personality traits {self.company.ceo.config.traits}.
...
You're reaching the end of your planned discussion structure."""
            ...
            spec = entity.free_action_spec(
                call_to_action=f"""You are {self.company.ceo.config.name}, the CEO leading this {meeting_type} with Big Five personality traits {self.company.ceo.config.traits}.
...
FINAL THEME: [The theme to use - either your original theme or a new theme]"""
    ...
    def _get_shareholder_speech(self, meeting_id, current_round, shareholder, discussion_log, round_log):
        ...
        spec = entity.free_action_spec(
            call_to_action=f"""You are {shareholder.config.name}, a shareholder in this investment company with Big Five personality traits {shareholder.config.traits}.
...
SPEECH CONTENT: [Your speech of 2-3 sentences if you decided to speak]"""
    ...
    def _get_ceo_response_to_shareholder(self, meeting_id, current_round, shareholder, discussion_log, round_log,
        ...
        spec = entity.free_action_spec(
            call_to_action=f"""You are {self.company.ceo.config.name}, the CEO leading this discussion with Big Five personality traits {self.company.ceo.config.traits}.
...
===MEETING CONTEXT===""",
    ...
    def _reassess_position_for_ceo(self, meeting_id, meeting_type, current_round, options, round_log):
        ...
        reassessment_prompt = f"""You are {ceo_name}, the CEO of the investment company with Big Five personality traits {self.company.ceo.config.traits}.
...
===MEETING CONTEXT==="""
    ...
    def _reassess_position_for_shareholder(self, shareholder, meeting_id, meeting_type, current_round, options,
        ...
        # Create reassessment prompt - modified to include confidence distribution
        reassessment_prompt = f"""You are {shareholder_name}, a shareholder in the investment company with Big Five personality traits {shareholder.config.traits}. 
...
Focus only on what was actually said during the discussion, not assumptions about unstated positions."""
        ...
        spec = entity.free_action_spec(
            call_to_action=f"""You are {ceo_name}, the CEO of the investment company with Big Five personality traits {self.company.ceo.config.traits}. 
...
The CEO has now proposed an alternative option: {proposal}"""
            ...
            vote_prompt = f"""You are {shareholder_name}, a shareholder in this investment company with Big Five personality traits {shareholder.config.traits}.
...
Vote according to your authentic assessment, guided by your Big Five personality traits and your judgment of what will best serve the company in this {market_condition} market.
...
You have now proposed an alternative option: {proposal}
        ...
        ceo_vote_prompt = f"""You are {ceo_name}, the CEO with Big Five personality traits {self.company.ceo.config.traits}.
...
"""
    ...
    def _get_ceo_decision_after_failed_vote(self, meeting_id, meeting_type, rejected_proposal):
        ...
        spec = entity.free_action_spec(
            call_to_action=f"""You are {self.company.ceo.config.name}, the CEO of the investment company with Big Five personality traits {self.company.ceo.config.traits}. 
...
Based on your Big Five personality traits ({self.company.ceo.config.traits}), consider:
...
REASON: [Your explanation that reflects your Big Five personality traits and leadership style]
...
This is a critical opportunity to show adaptive leadership while staying true to your core vision and Big Five personality traits. You need to select an alternative proposal that addresses key shareholder concerns while still aligning with your assessment of what's best for the company.
...
4. What option aligns best with your Big Five personality traits's approach to compromise?
...
1. Acknowledges the outcome of the vote in a way authentic to your Big Five personality traits
...
3. How does your Big Five personality traits ({self.company.ceo.config.traits}) influence your final decision-making approach in this challenging situation?
...
After you've submitted your assessments, I'll address the key themes and outline my thoughts on how we can improve in the coming year."""
    ...
    def _collect_shareholder_submissions(self, meeting_id, ceo_statement):
        ...
        def _submission_worker(shareholder_info):
            ...
            spec = entity.free_action_spec(
                call_to_action=f"""You are {shareholder_name}, a shareholder in the investment company with Big Five personality traits {shareholder.config.traits}.
    ...
    Based on your Big Five personality traits and the company's performance this year, provide your assessment focusing on SPECIFIC EXAMPLES rather than general impressions:
...
Respond in a way that authentically reflects your Big Five personality traits - whether that means being analytical, empathetic, decisive, or visionary.
...
=====================""",
    ...
    Based on your Big Five personality traits and your experience with the company this year, provide ratings for each dimension on a 1-5 scale. For each rating, provide a brief 1-sentence explanation that references specific examples from this year.
    IMPORTANT: Address all meeting participants in first person ("I/my"). Never refer to yourself by name or in third person. Maintain a professional tone suitable for addressing the entire group.
    Your response MUST use this exact format:
...
1. Acknowledges the ratings in a way authentic to your Big Five personality traits
2. Addresses both strengths and areas for improvement
3. Outlines 2-3 specific commitments for improvement in the coming year
4. Ends with a forward-looking vision that inspires confidence
...
=====================""",
    ...
    """
        ...
        reflection_prompt = f"""As {agent.config.name} with Big Five personality traits {mbti_type} and role {role}, reflect on how your thinking has evolved through this discussion round.
...
class Simulation:
    ...
    def __init__(self, agent_num, random_seed=None, model=None, result_dir="result/demo", bfi_type=None, team_composition=None):
        ...
        self.BackgroundInfo = BackgroundInfo(agent_num - 1, self.profile['ceo']['name'])
        ...
        ceo_bfi=self.ceo.config.traits
        ceo_bfi_type=self.ceo.config.extras['type']
        self.simulation_folder = self.result_saver.create_simulation_folder(agent_num,ceo_bfi=ceo_bfi_type)

        self.market = Market(seed=random_seed)
        print('Setup market')
    ...
    def setup_profile(self, specific_bfi_type=None, team_composition=None):
        """Set up agent profiles for either homogeneous or heterogeneous teams."""
        profile = dict()

        if specific_bfi_type:
            print(f"Setting up homogeneous team with OCEAN type: {specific_bfi_type}")
            profile['all'] = ChooseProfile(
                agent_num=self.agent_num,
                bfi_type=specific_bfi_type,
                random_seed=self.random_seed
            )
            profile['shareholders'] = profile['all'].copy()
            profile['ceo'] = SetupLeader(profile['shareholders'], random_seed=self.random_seed)
            if profile['ceo'] in profile['shareholders']:
                profile['shareholders'].remove(profile['ceo'])

        elif team_composition:
            print(f"Setting up heterogeneous team with composition: {team_composition}")
            from collections import Counter
            ceo_bfi_type = team_composition.get('ceo')
            shareholder_bfi_types = team_composition.get('shareholders', [])
            all_required_types = [ceo_bfi_type] + shareholder_bfi_types
            if len(all_required_types) != self.agent_num:
                raise ValueError(
                    f"Team composition size ({len(all_required_types)}) does not match agent number ({self.agent_num}).")

            bfi_type_counts = Counter(all_required_types)
            profile_pools = {}
            for bfi_type, count in bfi_type_counts.items():
                profiles_for_type = ChooseProfile(
                    agent_num=count,
                    bfi_type=bfi_type,
                    random_seed=self.random_seed
                )
                if len(profiles_for_type) < count:
                    raise ValueError(
                        f"Not enough unique profiles for OCEAN type '{bfi_type}'. Needed {count}, found {len(profiles_for_type)}.")
                profile_pools[bfi_type] = profiles_for_type
            ceo_profile = profile_pools[ceo_bfi_type].pop(0)
            profile['ceo'] = ceo_profile
            profile['shareholders'] = []
            for bfi_type in shareholder_bfi_types:
                shareholder_profile = profile_pools[bfi_type].pop(0)
                profile['shareholders'].append(shareholder_profile)
            profile['all'] = [profile['ceo']] + profile['shareholders']

        else:
            raise ValueError(
                "Either 'specific_bfi_type' (for homogeneous) or 'team_composition' (for heterogeneous) must be provided.")
    ...
    def run(self, years=5):
        ...
        simulation_header += f"- CEO: {self.ceo.config.name} (OCEAN Trait: {self.ceo.config.traits})\n"
        ...
        for name, shareholder in self.shareholders.items():
            simulation_header += f"- {name} (OCEAN Trait: {shareholder.config.traits})\n"
        ...
        print(f"CEO: {self.ceo.config.name} (OCEAN Trait: {self.ceo.config.traits})")
        print("Shareholders:")
        for name, shareholder in self.shareholders.items():
            print(f"- {name} (OCEAN Trait: {shareholder.config.traits})")
        ...
        results = {
            ...
            "meta": {
                ...
                "ceo": {
                    ...
                    "ocean_trait": self.ceo.config.traits
                },
                "shareholders": [{
                    "name": shareholder.config.name,
                    "ocean_trait": shareholder.config.traits
        ...
        import os
        decision_tracker_filename = os.path.join(self.simulation_folder, f"decision_tracker_data_{ceo_bfi_type}.json")
...
def run_simulation(team_name, team_composition, random_seed=42, num_agents=6, years=1):
    """Run a single simulation for a specific team composition"""
    result_dir = f"result/Heterogeneous"

    print(f"Starting simulation for team: {team_name}")

    simulation = Simulation(
        agent_num=num_agents,
        random_seed=random_seed,
        model=model,
        result_dir=os.path.join(result_dir, team_name),
        team_composition=team_composition
    )
    ...
    try:
        folder_path = simulation.simulation_folder
        team_info_path = os.path.join(folder_path, 'team_composition.txt')

        with open(team_info_path, 'w', encoding='utf-8') as f:
            f.write(f"Team Name: {team_name}\n\n")
            f.write("Team Composition (OCEAN Types):\n")
            f.write("=" * 30 + "\n")

            if isinstance(team_composition, dict):
                f.write(f"CEO: {team_composition.get('ceo')}\n\n")
                f.write("Shareholders:\n")
                for i, bfi_type in enumerate(team_composition.get('shareholders', [])):
                    f.write(f"  - Shareholder {i + 1}: {bfi_type}\n")
            else:
                f.write(f"Homogeneous Type: {team_composition}\n")

        print(f"SUCCESS: Team composition info saved to: {team_info_path}")
    except Exception as e:
        print(f"WARNING: Could not save team composition file. Error: {e}")
...
def main():
    ...
    HETEROGENEOUS_TEAMS = {
        "CognitiveFrictionTeam": {
            "ceo": "OceAn",
            "shareholders": ["oCEan", "OceAn", "oCEan", "oceAn", "oCEAN"]
        },
        "RoleComplementaryTeam": {
            "ceo": "OCEAN",
            "shareholders": ["OcEaN", "oCEan", "ocEAn", "oCean", "oCEan"]
        },
        "RiskBalancingTeam": {
        ...
        "HighAgreeablenessTeam": {
            "ceo": "OCEAn",
            "shareholders": ["OcEAn", "oCEAn", "ocEAn", "oCeAn", "OceAn"]
        },
        "LeaderModeratedFrictionTeam": {
            "ceo": "OCEAN",
            "shareholders": [ "OceAn", "oCEan", "oceAn", "oCEAN", "OceAn" ]
        },
        "IntellectualPowerhouseTeam": {
            "ceo": "OCEAn",
            "shareholders": ["OCean", "OCEan", "OCEAn", "OCean", "OCean" ]
        },
        "IntrovertExtrovertBalanceTeam": {
            "ceo": "OCEAn",
            "shareholders": [ "OcEan",  "ocEAn",  "OCean",  "oCean",  "OceAn"]
        },
        "CriticalDebatersTeam": {
            "ceo": "OCEan",
            "shareholders": ["OcEan",  "oCEan", "OceAn", "oCean", "ocean" ]
        }
    ...
    print(f"Running {len(HETEROGENEOUS_TEAMS)} heterogeneous team simulations in parallel...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        future_to_team = {
            executor.submit(run_simulation, team_name, team_comp, random_seed, num_agents, years): team_name
            for team_name, team_comp in HETEROGENEOUS_TEAMS.items()
        }
        ...
        for future in concurrent.futures.as_completed(future_to_team):
            ...
            try:
                completed_team_name, results = future.result()
                all_results[completed_team_name] = results
                print(f"Successfully completed simulation for {completed_team_name}")
            except Exception as exc:
                print(f"Team {team_name} generated an exception: {exc}")

    # Print summary of all results
    print("\n=== FINAL SUMMARY ===")
    print("Team Name             | Final Assets | Growth Rate")
    print("-" * 55)
    for team_name, results in sorted(all_results.items()):
        print(f"{team_name:21} | ${results['final_assets']:11.2f} | {results['growth_percentage']:6.2f}%")
